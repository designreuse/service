...
TB.form.DistrictSelector = new function () {
    var D = {
        filter: function (F, E) {
            return true
        }
    ...
    var B = function (I, H, F, E) {
        ...
        if (G && G.children) {
            G.children.forEach(function (L) {
                ...
                I.options[I.options.length] = new Option(K.name, K.code)
            })
        }
        C(I)
    };
    ...
    this.attach = function (E, I, H, F) {
        E = $(E);
        I = $(I);
        H = $(H);
        F = TB.applyIf(F || {}, D);
        var G = {};
        G.selectProv = function (K, J) {
            if (K) {
                E.value = K
            }
            I.options.length = 0;
            H.options.length = 0;
            B(I, E.value, F.filter, 2);
            if (typeof J == "undefined" || J == true) {
                this.selectCity(I.value)
            }
            C(E)
        };
        G.selectCity = function (K, J) {
            if (K) {
                I.value = K
            }
            if (typeof J == "undefined" || J == true) {
                H.options.length = 0;
                B(H, I.value, F.filter, 3)
            }
            C(I)
        };
        G.selectDist = function (M, J) {
            if (M == "1") {
                return
            }
            if (M) {
            ...
            if (typeof J == "undefined" || J == true) {
                ...
                if (L == "1") {
                    ...
                    if (K == "1") {
                        ...
                        G.selectProv(K, true);
                        G.selectCity(L, true);
                        G.selectDist(M, false)
                    }
                }
            }
            C(H)
        };
        G.initSelectedCodes = function (J) {
            if (YAHOO.lang.isArray(J)) {
                G.selectProv(J[0] || "", true);
                G.selectCity(J[1] || "", true);
                G.selectDist(J[2] || "", false)
            } else {
                if (YAHOO.lang.isString(J)) {
                    I.options[0] = new Option("\u8bf7\u9009\u62e9\u5e02...", "");
                    H.options[0] = new Option("\u8bf7\u9009\u62e9\u533a/\u53bf...", "");
                    C(I);
                    C(H);
                    G.selectDist(J, true)
                }
        ...
        G.getSelectedCodes = function () {
            return [E.value, I.value, H.value]
        };
        G.getDivisionText = function (K) {
            var J = [E, I, H].map(function (L) {
                if (L.selectedIndex >= 0) {
                    return L.options[L.selectedIndex].text
                }
            });
            return J.join(K || "")
        };
        E.options[0] = new Option("\u8bf7\u9009\u62e9...", "");
        B(E, "1", F.filter, 1);
        $E.on(E, "change", function () {
            G.selectProv()
        });
        $E.on(I, "change", function () {
            G.selectCity()
        });
        $E.on(H, "change", function () {
            C(H)
        });
        return G
    };
    this.initData = function (G) {
        ...
        for (var F in G) {
            ...
            var H = G[F], E = H[1] || "1";
            A[F] = {name: H[0], code: F, parent: H[1]};
            if (A[E]) {
                if (!A[E]["children"]) {
                    A[E]["children"] = []
                }
...
TB.form.DistrictSelector._tb_ds_data = {
    "110000": ["\u5317\u4eac", "1"],
    "110100": ["\u5317\u4eac\u5e02", "110000"],
    "110101": ["\u4e1c\u57ce\u533a", "110100"],
    "110102": ["\u897f\u57ce\u533a", "110100"],
...
TB.form.DistrictSelector.initData(window.tdist || TB.form.DistrictSelector._tb_ds_data);