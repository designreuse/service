...
import com.lives.manage.web.entity.Page;

/**
 *
 * 分页拦截器，用于拦截需要进行分页查询的操作，然后对其进行分页处理。 利用拦截器实现Mybatis分页的原理： 要利用JDBC对数据库进行操作就必须要有一个对应的Statement对象，Mybatis在执行Sql语句前就会产生一个包含Sql语句的Statement对象，而且对应的Sql语句
 * 是在Statement之前产生的，所以我们就可以在它生成Statement之前对用来生成Statement的Sql语句下手。在Mybatis中Statement语句是通过RoutingStatementHandler对象的
 * prepare方法生成的。所以利用拦截器实现Mybatis分页的一个思路就是拦截StatementHandler接口的prepare方法，然后在拦截器方法中把Sql语句改成对应的分页查询Sql语句，之后再调用
 * StatementHandler对象的prepare方法，即调用invocation.proceed()。 对于分页而言，在拦截器里面我们还需要做的一个操作就是统计满足当前条件的记录一共有多少，这是通过获取到了原始的Sql语句后，把它改为对应的统计语句再利用Mybatis封装好的参数和设
...
public void setProperties(final Properties properties) {
	 ...
	 * @return
	 */
    private String getPageSql(final Page<?> page, final String sql) {
	 ...
	 * @return Mysql数据库分页语句
	 */
    private String getMysqlPageSql(final Page<?> page, final StringBuffer sqlBuffer) {
	 ...
	 */
    private String getOraclePageSql(final Page<?> page, final StringBuffer sqlBuffer) {
		// 计算第一条记录的位置，Oracle分页是通过rownum进行的，而rownum是从1开始的
       final int offset = (page.getPageNo() - 1) * page.getPageSize() + 1;
	 ...
	 */
    private void setTotalRecord(final Page<?> page,
           final MappedStatement mappedStatement, final Connection connection) {
