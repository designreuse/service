...
public class RedisService {
    ...
    public void setMultiToken(String userId, String token) {
        Jedis jedis = null;
        boolean isBroken = false;
        try {
            jedis = this.getJedis();
            Transaction transaction = jedis.multi();
            transaction.hset(userId, "token", token);
            transaction.hset(token, "userId", userId);
            transaction.hset(token, "isOffline", "0");
            transaction.exec();
        } catch (Exception e) {
            isBroken = true;
        } finally {
